...
for tid in [14]:
	...
	for idx in indexlist:
		for ix in range(idx-2,df.shape[0]):
			...
			for jx in range(0,df.shape[1],2):
				if df.iloc[ix,0]==idx or df.iloc[ix,jx-1]==idx:
...
colors = ['#0000CD','#EE3B3B','#8EE5EE','#FF6103','#458B00','#FFB90F','#006400','#B23AEE','#00BFFF','#00C957','#8B6914','#FF1493','#8FBC8F','#CD661D','#8B8878','#FF7256','#0000CD','#EE3B3B','#8EE5EE','#FF6103','#458B00','#FFB90F','#E06E00','#B23EEE','#E0BFFF','#0EC957','#8E6914','#FA1493','#EFBC8F','#CE661D','#8E8878','#FE7256']

for index,ft in enumerate(ftlst):
	fig = plt.figure(figsize=(32,18))
	ax = plt.subplot()
	ax.set_xlim(0,max(ft[2])+2)
	...
	for ind,itm in enumerate(ft[0]):
		if ind != 0 and ((ft[2][ind]-ft[1][ind]) < 4):
	...
	for i,j in enumerate((ft[0])):
		if i not in notplottedlist:   # min time filter
			for k in range(ft[1][i]-1,ft[2][i]):
				...
				ax.text(ft[2][i]+1,i+1, str(ft[0][i]),fontsize=20)
			plt.plot()
	...
	filename = saveFolder + '/'+'FT_ID_'+str(ft[0][0])+'.png'
